...
<<<<<<< HEAD
 public:
   ...
   public:
    ...
    void add_edge_id(const EdgeId& Next_id) {
      for (const auto& edge_id_i : edge_ids_)
        if (edge_id_i != Next_id)
          assert("Edge id already exist");
  ...
  class Edge {
   public:
    Edge(const pair<VertexId, VertexId>& new_vertex, const EdgeId& edge_id)
        : vertex_ids(new_vertex), id(edge_id) {}
  ...
  void add_edge(const VertexId& first, const VertexId& second) {
    ...
    const auto& new_edge = edges_.emplace_back(
        pair<VertexId, VertexId>{first, second}, get_nextedge());
  ...
  /*	 string makeJSON() const
           {
                   ...
                   {
                           ss<< "{\"id\":" <<vertices_[i].id <<
     ",\"edge_ids\":["; for (int j = 0; j < vertices_[i].edge_ids_.size() - 1;
     j++)ss <<vertices_[i].edge_ids_[j] << ","; ss
     <<vertices_[i].edge_ids_[vertices_[i].edge_ids_.size() - 1] << "]}"; if (i
     != vertices_.size() - 1) ss << ",\n";
                   }
                   ss << "\n],\"edges\":[";
                   for (int i = 0; i < edges_.size(); i++)
                   {
                          ss<< "{\"id\":" <<edges_[i].id << ",\"vertex_ids\":["
     << edges_[i].vertex_ids.first<< "," << edges_[i].vertex_ids.second << "]}";
 ...
 public:
  ...
  void print_vertex(const vector<Graph::Vertex>& vertices_) {
    ...
    for (; i < vertices_.size(); i++) {
      ...
      for (int j = 0; j < vertices_[i].edge_ids_.size() - 1; j++)
        ss << vertices_[i].edge_ids_[j] << ",";
...
int main() {
  Graph graph{};
  pair<VertexId, VertexId> G[EDGE_COUNT] = {
...
int main()
{
	  ...
	  file.close();
	  return 0;
>>>>>>> fb99aae48ef1e53cb768caaf7dacdbb80ab41d7e
}
